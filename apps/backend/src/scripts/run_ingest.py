...
Usage: make ingest HANDLES=handle1,handle2 COUNT=100
   or: python -m src.scripts.run_ingest handle1,handle2 [count]
"""
...
def main() -> None:
    if len(sys.argv) < 2 or not sys.argv[1].strip():
        print("usage: run_ingest.py handle1,handle2,... [count]", file=sys.stderr)
        raise SystemExit(1)
    ...
    try:
        for handle in handles:
            account_id = ingest_account(db, handle, count)
