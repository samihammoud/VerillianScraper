...
def run_query(db: Session, query: CrawlQuery) -> tuple[int, int]:
    ...
    for handle in handles[:ACCOUNTS_PER_QUERY]:
        try:
            #ingest account stores video to disk
            ingest_account(
...
def run_round(db: Session, world_slug: str) -> bool:
    ...
    #only after full round queries are passed, call the vlm
    #only 2500 videos are grabbed to describe, rest are lift on disk - leftover for next round
